"""train_transformer.py — Encoder-Decoder Transformer training"""
...
def main() -> None:
    ...
    model     = TransformerDateGenerator(tokenizer.cond_vocab_size, tokenizer.date_vocab_size).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if vl < best_val:
            best_val = vl
            torch.save(model.state_dict(), save_dir / "transformer_best.pt")

    plt.figure()
    plt.plot(tr_losses, label="train"); plt.plot(vl_losses, label="val")
    plt.xlabel("Epoch"); plt.ylabel("Loss"); plt.legend()
    plt.savefig(save_dir / "transformer_loss_curve.png"); plt.close()
